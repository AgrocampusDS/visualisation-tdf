...
ggplot(data=tdf_bis,aes(as.numeric(Year))) +
  geom_bar(position = "fill",width = 1,alpha =0.7,aes(fill = Type))+

  geom_smooth(aes(y=n/25),linetype='dashed',colour='black',se=F)+
  
  scale_fill_hue(c=60, l=80)+
  scale_y_continuous(sec.axis = sec_axis(trans = ~.*25, name = "Number of stages")) +
  geom_rect(aes(xmin = 1914, xmax = 1919, ymin = 0, ymax = 1),fill='white')+
  
  geom_text(aes(1917,0.5), label = "world war I", angle =270, size = 4,colour='black')+
  geom_rect(aes(xmin = 1939, xmax = 1947, ymin = 0, ymax = 1),fill='white')+
